namespace interChallenge01 {
  ...
  class Persons {
    ...
    constructor(id: number, content: string, createdAt: string, score: number, img: string, username: string, replyingTo?: string){
      this.id = id
  ...
  fetch("data.json")
    ...
    .then((data: personsComment) => {
      let amy: personType = data.comments[0]
      let max: personType = data.comments[1]
      let ramses: personTypeInto | undefined = data.comments[1].replies?.[0]
      let julius: personTypeInto | undefined = data.comments[1].replies?.[1]
      ...
      const addContent = (person: Persons, replies: number, input?: string): any =>{
        ...
        if (replies == 1) {
          ...
          const contResp = document.querySelector(".container-response") as HTMLDivElement;
          if (contResp) {
      ...
      const commentBox = (person: Persons, element: HTMLElement)=>{
      ...
      const container = document.querySelectorAll(".container") as NodeListOf<HTMLDivElement>
      const container2 = document.querySelectorAll(".container-2") as NodeListOf<HTMLDivElement>

      const reply = document.querySelectorAll(".reply-text") as NodeListOf<HTMLParagraphElement>
      const reply2 = document.querySelectorAll(".reply-text-2") as NodeListOf<HTMLParagraphElement>

      const addComment = (botton: NodeListOf<HTMLElement>, div: NodeListOf<HTMLElement>)=>{
      ...
      const textareaBox = document.querySelectorAll(".input") as NodeListOf<HTMLTextAreaElement>
      const sendBotton = document.querySelectorAll(".send") as NodeListOf<HTMLParagraphElement>
      const containerResponse = document.querySelector(".container-response") as HTMLDivElement

      const clickSendBotton = (dom: HTMLElement) =>{
        dom.addEventListener("click", (e)=>{
          const target = e.target as HTMLElement
          if (target.classList.contains("send")) {
            const inputContainer = document.querySelector(".container-input") as HTMLDivElement
      ...
      const countNumber = document.querySelectorAll(".count-number") as NodeListOf<HTMLParagraphElement>
